...
### Worker / Redis hosting (2026-05-07)

**BullMQ + Upstash free tier is structurally infeasible**
- 2026-05-07 — A BullMQ Worker with `concurrency: 5` issues blocking pulls on every concurrency slot. Idle floor is ~86K Redis commands/day. Tuning to `concurrency: 1, drainDelay: 30, stalledInterval: 5min, HEARTBEAT: 5min` reduces it but **does not get under the Upstash 500K/month free cap**.
- 2026-05-07 — The hidden cost is Upstash counts every Lua sub-command separately. BullMQ's main poll script (`EVALSHA`) runs 7 commands inside the Lua block: `HMGET bull:events:meta`, `ZRANGEBYSCORE bull:events:delayed`, `GET bull:events:limiter`, `RPOPLPUSH bull:events:wait bull:events:active`, `ZPOPMIN bull:events:prioritized`, `DEL bull:events:pc`, `ZRANGE bull:events:delayed`. Plus `BZPOPMIN bull:events:marker`. Total ~9 cmds per 30s = ~26K/day = **~780K/month** with zero traffic. Stalled-check every 5 min adds 6 more per fire. Upstash's 500K free monthly cap is exhausted in ~19 days of idle uptime, less with traffic.
- 2026-05-07 — **Conclusion:** Upstash free is a poor host for any continuously-running BullMQ Worker. The polling model (correct for low-latency job pickup) fights pay-per-command pricing. Either switch to memory-billed Redis (Railway addon, $0 inside $5 hobby plan) or Upstash pay-as-you-go (~$0.20 per extra 100K, likely <$2/month at our scale).
- 2026-05-07 — **Decision:** AutomateBro/BloomDM uses Railway Redis as the primary Redis backend. The Upstash database is kept around as a fallback (pause-don't-delete) so we can swap `REDIS_URL` back in an emergency. See `docs/redis-on-railway.md` for the migration + swap-back runbook.
- 2026-05-07 — Tuned worker config (`concurrency: 1, drainDelay: 30, stalledInterval: 5min, heartbeat: 5min`) is kept on Railway Redis too. The math for free Upstash forced us to find these values; on Railway Redis they're cheap insurance against any future Redis-billed migration. To revert when real volume justifies it: bump `WORKER_CONCURRENCY` back to 5 and remove the `drainDelay`/`stalledInterval` overrides.
